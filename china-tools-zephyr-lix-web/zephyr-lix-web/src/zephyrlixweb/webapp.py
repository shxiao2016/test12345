...
from zephyrlixweb.models import db, LixForZephyrTracking, LixSubscriber, LixDetailInfo, LixNewlyIntroduced, \
    LixLatestTimestampInfo, LixFullyRampedStatusTable, FavoriteLix, UploadedLix
from constants import PLATFORMS, PILLARS, OPTION_DAYS, OPTION_BETWEEN, FULLY_RAMPED_TIME_RANGE, KEY_VAGUE_SEARCH, \
    NON_FULLY_RAMPED, SEPARATOR, HEADERS, SPRINT_VERSION, DELETE_SUCCESSFULLY
from email_sender import send_voyager_newly_introduced_lix_report, get_voyager_lix_ramp_status_between
from utils import fetch_newly_intro_detailed_info
from upload_lix import validate_and_store_info, add_to_favorite_list
from linkedin.web import create_app
from linkedin.config.base import framework
from linkedin.flask_sqlalchemy.cfg2 import FlaskSQLAlchemy
...
app.config['SQLALCHEMY_DATABASE_URI'] = framework.configured_plugin(FlaskSQLAlchemy).SQLALCHEMY_DATABASE_URI
# connection in the connection pool will get refreshed every 3600 seconds.
app.config['SQLALCHEMY_POOL_RECYCLE'] = 3600
# There are 20 connections in the connection pool.
app.config['SQLALCHEMY_POOL_SIZE'] = 20

db.app = app
db.init_app(app)
app.register_blueprint(sched, url_prefix='/sched')
...
def ramped_status_change():
    ...
    voyager_lixes_ramp_between_ranges = get_voyager_lix_ramp_status_between(start_timestamp, to_timestamp,
                                                                            favorite_list)
...
def my_watch_list():
    ...
    watch_list = []
    unique_set = []
    join_res_for_watch_list = db.session.query(FavoriteLix, LixDetailInfo, LixLatestTimestampInfo).filter(
        FavoriteLix.name == LixDetailInfo.name).filter(
        LixDetailInfo.timestamp == LixLatestTimestampInfo.latest_timestamp).filter(
        FavoriteLix.author == user).filter(FavoriteLix.is_zephyr_repo_lix == 1).all()
    for join_item in join_res_for_watch_list:
        # join_item[1] is a LixDetailInfo object.
        if join_item[1].name not in unique_set:
            watch_list.append(join_item[1])
            unique_set.append(join_item[1].name)

    join_res_for_uploaded_key = db.session.query(FavoriteLix, UploadedLix).filter(
        FavoriteLix.author == user).filter(UploadedLix.name == FavoriteLix.name).filter(
        FavoriteLix.is_zephyr_repo_lix == 0).all()
    for join_item in join_res_for_uploaded_key:
        if join_item[1].name not in unique_set:
            watch_list.append(join_item[1])
            unique_set.append(join_item[1].name)
    return render_template("my_watch_list.html", lix_detail_records=watch_list, PLATFORMS=PLATFORMS, PILLARS=PILLARS)
...
def add_to_watch_list():
    ...
    if checked == 'true':
        ...
        favorite_lix = db.session.query(FavoriteLix).filter(FavoriteLix.name == lix_name).filter(
            FavoriteLix.author == user).first()
        if favorite_lix:
            favorite_lix.delete()
    return Response("good", status=200)


@app.route('/upload_a_lix/', methods=['POST'], endpoint='upload_a_lix')
def upload_a_lix():
    user = authenticate()
    if not user:
        return Response('Please log in first!')
    lix_name = request.form['lix_name']
    message = validate_and_store_info(lix_name, user)
    return Response(message)


@app.route('/delete_a_lix/', methods=['POST'], endpoint='delete_a_lix')
def delete_a_lix():
    user = authenticate()
    if not user:
        return Response('Please login first!', status=500)
    lix_name = request.form['lix_name']
    favorite_lix = db.session.query(FavoriteLix).filter(FavoriteLix.name == lix_name).filter(
        FavoriteLix.author == user).first()
    if favorite_lix:
        favorite_lix.delete()
    return Response(DELETE_SUCCESSFULLY)
...
def get_user_favorite_lix(user=None):
    ...
    res = db.session.query(FavoriteLix).filter(FavoriteLix.author == user).filter(FavoriteLix.is_zephyr_repo_lix == 1).all()
    for item in res:
...
def authenticate():
    """
    :return str|None: if it is a user, then return the user alias. Otherwise return None.
    """
