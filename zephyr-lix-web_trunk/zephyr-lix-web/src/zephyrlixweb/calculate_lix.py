...
from constants import WORK_DIR, ZEPHYR_GIT_REPOS_LOCAL, LIX_FILE_PATTERN, ZEPHYR_MP_VERSION_FILE, \
    ZEPHYR_MP_VERSION_KEY_WORDS, FP_LESS_THAN_10, FP_BEYOND_10, TEN_DAYS_MILLI_SECONDS, LIX_STRING_PATTERN, \
    PROFILE_VOYAGER_PILLARS, CAREER_VOYAGER_PILLARS, THIRTY_DAYS_MILLI_SECONDS, SIXTY_DAYS_MILLI_SECONDS, \
    CONTENT_VOYAGER_PILLARS, GROWTH_VOYAGER_PILLARS, NINETY_DAYS_MILLI_SECONDS, NEWER_THAN_30, GREATER_THAN_30, \
    GREATER_THAN_60, GREATER_THAN_90, UNKNOWN_PERIOD, PLATFORMS
...
def checkout_code_via_download():
    ...
    git_repos_local = ZEPHYR_GIT_REPOS_LOCAL
    repo = repository(repo_path=WORK_DIR)
    repo_dir = repo.mkdir()
    log.info(msg='code repository location: %s' % repo_dir)
    for platform, mp in PLATFORMS.iteritems():
        rev = repo.get_latest_revision_in_git(mp)
        timestamp = totimestamp(dt=datetime.datetime.utcnow())
        extracted_folder = '{0}/{1}_{2}'.format(WORK_DIR, mp, timestamp)
    ...
    log.info("########finish checkout code!!#######")


def find_lix_files(platform):
    local_repo = ZEPHYR_GIT_REPOS_LOCAL[platform]
    if isinstance(LIX_FILE_PATTERN[platform], str):
        return [os.path.join(local_repo, LIX_FILE_PATTERN[platform])]
    elif isinstance(LIX_FILE_PATTERN[platform], list):
        return get_files_via_list(local_repo, LIX_FILE_PATTERN[platform])
    else:
        return get_files_via_pattern(local_repo, LIX_FILE_PATTERN[platform])
...
def get_mp_version(platform):
    result = "0"
    if platform not in ZEPHYR_GIT_REPOS_LOCAL:
        return result
    filename = ZEPHYR_MP_VERSION_FILE[platform]
    keyword = ZEPHYR_MP_VERSION_KEY_WORDS[platform]
    file_path = '{0}/{1}'.format(ZEPHYR_GIT_REPOS_LOCAL[platform], filename)
...
def get_all_lix_details():
    log.info("########begin daily scheduler!!###########")
    zephyr_lix_from_code = get_all_lix()
    for platform in zephyr_lix_from_code:
        log.info("###########begin parsing lix###############")
        raw_data = fetch.get_all_data(zephyr_lix_from_code[platform])
        calc_detail(raw_data, platform)
        shutil.rmtree(ZEPHYR_GIT_REPOS_LOCAL[platform], ignore_errors=True)
    ...
    for platform in PLATFORMS:
        ...
        for lix_file in lix_files:
            with file(lix_file) as f:
                lines = f.readlines()
                for line in lines:
                    m = LIX_STRING_PATTERN[platform].search(line)
                    if m:
                        lix_str = m.group(1)
                        ...
                        if (len(split_array) >= 3) and (
                            ...
                            if tpillar in PROFILE_VOYAGER_PILLARS:
                                tpillar = "profile"
                            elif tpillar in CAREER_VOYAGER_PILLARS:
                                tpillar = "career"
                            elif tpillar in CONTENT_VOYAGER_PILLARS:
                                tpillar = "content"
                            elif tpillar in GROWTH_VOYAGER_PILLARS:
                                tpillar = "growth"
                            else:
                                tpillar = "others"
                        else:
                        ...
                        if tpillar == "others":
                            ...
                            for pillar in PROFILE_VOYAGER_PILLARS:
                                if lix_str_lower.find(pillar) != -1:
                                    tpillar = "profile"
                            for pillar in CAREER_VOYAGER_PILLARS:
                                if lix_str_lower.find(pillar) != -1:
                                    tpillar = "career"
                            for pillar in CONTENT_VOYAGER_PILLARS:
                                if lix_str_lower.find(pillar) != -1:
                                    tpillar = "content"
                            for pillar in GROWTH_VOYAGER_PILLARS:
                                if lix_str_lower.find(pillar) != -1:
                                    tpillar = "growth"
...
def calc_detail(raw_data, platform):
    log.info("#######store lix detail for platform {0}!######".format(platform))
    current_milli_time = int(round(time.time() * 1000))
    ten_day_cutoff = current_milli_time - TEN_DAYS_MILLI_SECONDS
    thirty_day_cutoff = current_milli_time - THIRTY_DAYS_MILLI_SECONDS
    sixty_day_cutoff = current_milli_time - SIXTY_DAYS_MILLI_SECONDS
    ninety_day_cutoff = current_milli_time - NINETY_DAYS_MILLI_SECONDS
    ...
    for key, values in raw_data.iteritems():
        ...
        for value in values:
            ...
            if "name" not in value:
                value['name'] = "unknown_name"
                lix_labels = "unknown_labels"
                latest_exp_fabric = "unknown_fabric"
                latest_exp_spec = "unknown_spec"
                last_active_spec = "unknown_spec"
                latest_experiment_id = "unknown_experiment_id"

            else:
                lix_labels = get_labels.get_lix_labels(value['name'])
                latest_exp_fabric, latest_exp_spec = get_segment_details(value['name'])
                last_active_spec, latest_experiment_id = get_latest_active_spec(value['name'])
                is_zephyr_lix = value['name'].startswith("zephyr")
                if is_zephyr_lix:
            ...
            ramped_by_mpname = get_ramp_by_MP(last_active_spec)
            if value["name"] not in lixes_list:
                lixDetailInformation = LixDetailInfo(timestamp=cur_time,
                                                     ...
                                                     latest_exp_fabric=latest_exp_fabric,
                                                     spec=latest_exp_spec,
                                                     labels=lix_labels,
                                                     last_active_spec=last_active_spec,
                                                     ramped_by_mpname=ramped_by_mpname,
                                                     is_zephyr_lix=is_zephyr_lix,
