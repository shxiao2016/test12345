...
def get_lix_info(lix_name):
    ...
    cmd = 'curl -s -X GET "%s"' % fully_ramped_url
    # if lix_name == 'voyager.feed.android.control-menu-tooltip':
    #     cmd = ''
    # cmd = 'curl %s' % fully_ramped_url
    # fully_ramped = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    # stdout, stderr = fully_ramped.communicate()
    fully_ramped = subprocess.check_output(cmd, shell=True)
    # if stdout.lower().find('true') != -1:
    if fully_ramped.lower().find('true') != -1:
        ramped = 'True'
    else:
        ramped = 'False'
